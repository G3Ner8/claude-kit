...
| [`surveyor`](./skills/surveyor/) | 🗺️ surveyor | **between** cycles | Survey the real project state — reconcile declared status (backlog, status docs, tracker) against ground truth (merged history, MR/issue state, the code), report the drift, offer to sync stale docs. Answers "is the status right", not "what next" — it verifies, never trusts the doc. _(experimental)_ | "where are we — what's actually done?" |

Read-only deliverables — they produce a report / document and never touch code (`surveyor` can sync status docs, but only on explicit approval). Invoke individually (`/architect`, `/drafter`, `/detective`, `/inspector`, `/archivist`, `/surveyor`) from any project regardless of stack. The six line up across the lifecycle: **architect** designs the plan, **drafter** writes the work order, **detective** finds the cause, **inspector** gates the change, **archivist** preserves the lesson, **surveyor** surveys where it all stands.
