...
- **Phases** *(emit only when the plan carries a named multi-stage agent chain AND the target supports first-class phases, e.g. SDC)* — one `### Phase N: <title>` sub-block per stage, each carrying its own `agent-type:` / optional `model:` / `skills:` line (same format as Agent Configuration) plus that stage's instructions. Format:
  ...
  ### Phase 2: Harden
  agent-type: harden-agent
  Read the target repo's `.claude/agents/harden-agent.md` and follow it directly. One pass over
  the accumulated working-tree diff: DRY, consistency, cleanup.

  ### Phase 3: Verify gate
  agent-type: verify-agent
  Read the target repo's `.claude/agents/verify-agent.md` and follow it directly. Run the
...
- **Every Agent Configuration declaration is best-effort at runtime — pair it with a body instruction that forces use.** Pre-claim validation only checks the name exists; it never guarantees the agent delegates or invokes it mid-run. Confirmed empirically (a canary persona/skill run in a real SDC-onboarded repo): real `Agent`/`Skill`-tool invocation of a project-defined target fails every time ("not registered in this harness" / "Unknown skill"); reading the target file directly and applying it inline works every time. *(Verified 2026-07 — this is a snapshot of the harness, not a law. Tripwire: if any future SDC transcript shows a successful `Agent`/`Skill`-tool invocation of a project-defined target, this workaround is obsolete — re-run the canary and simplify this rule and the transitive rule below.)* So:
