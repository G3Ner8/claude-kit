...
| `{{STACK}}` | One-line stack summary for `implement` + `harden` agent `description:` frontmatter | `React 19 / TS / Vite / Tailwind / Radix` |
...
| `{{OUTPUT_LANG}}` | **Default** report language for `implement` + `harden` + `test` (`verify` is always English). Agents resolve the actual language per session via their `## Report language` section: explicit user request > dominant session language > this default | `Thai` |
| `{{BACKEND_NAME}}` | Backend project / repo name (used in implement Debug Protocol + verify drift-gate report wording) | `shop-api` |
...
| `{{STRUCTURE_EXTRACT_MAPPING}}` | (Optional) Project-specific extraction-kind → section bullet list for `harden` Structure check when extracting. Empty if generic | `- Schema extraction → Section 4.1 + 4.4 + 9`<br>`- Component split / extract → Section 4.2 + 6` |
...
| `{{REFERENCE_PAGE_EXAMPLES}}` | Comma-separated examples of reference (baseline) pages agents anchor on | `OrderListPage, ProductListPage, ProductDetailPage` |
...
| `{{PAGE_STATUS_REPORT_BLOCK}}` | The `## Page status` flip/regression report subsection (empty unless a page-status audit script exists) | (see PAGE_STATUS_REPORT_BLOCK template) |
| `{{ARCHITECTURE_DOCS_GLOB}}` | Glob of architectural docs (drives verify doc-sync gate) | `shop-web/docs/architecture/*` |
...
| `{{PLAN_FILE_PATTERN}}` | (Optional) Trailing parenthetical hint for Plan-file path convention in `implement` Mode table. Must be project-relative — never an absolute or author-personal path. Empty = the row reads "Plan file or skill output from an earlier turn" | ` (\`docs/plans/*-plan.md\`)` |
...
| `{{COMMIT_SCOPE_OPTIONS}}` | Scope-options sentence in `verify` commit-draft section | `` `(shop-web)` when diff is purely frontend · `(shop-api)` when purely backend · omit for cross-cutting / repo-level `` |
| `{{API_TRIGGER_HINT}}` | Inline phrase in `verify` Bug+regression scan describing what API surface touched looks like | `` `services/api.ts` or feature `api/` `` (default: `network surface`) |
...
| `{{TEST_CMD}}` | Test runner used by the verify "test-only diff" fast path and `shop-test` chunk runs (`{{TEST_CMD}} -- <files>`) | `cd shop-web && npm run test:unit` |
| `{{TEST_COV_CMD}}` | Coverage-variant test runner (`shop-test` baseline + delta capture) | `cd shop-web && npm run test:cov` |
| `{{FULL_CHECK_CMD}}` | Project's full check command, run by `verify` in verify mode (diff-review mode runs `{{BUILD_CMD}}` only). Falls back to `{{BUILD_CMD}}` if the project has no aggregate check script. Must NOT mutate the working tree (no `format` / `lint:fix`) | `cd shop-web && npm run check` |
...
| `{{PAGE_STATUS_AUDIT_SCRIPT_REF}}` | Either ` + skim <script> (PAGE_STATUS map)` or empty | ` + skim shop-web/scripts/page-polish-audit.mjs (PAGE_STATUS map)` |
| `{{PAGE_STATUS_CHECK_SECTION}}` | Full Page-status check block (or empty) | (see template — only rendered if a harden audit script is configured) |
| `{{PAGE_STATUS_AUDIT_CMD}}` | Command that emits the page status map (referenced inside `{{PAGE_STATUS_CHECK_SECTION}}`) | `cd shop-web && node scripts/page-polish-audit.mjs` |
| `{{PAGE_STATUS_AUDIT_SOURCE}}` | Source file path used as the `PAGE_STATUS` reference | `shop-web/scripts/page-polish-audit.mjs` |

## Backend

| Placeholder | What it is | Example |
|---|---|---|
| `{{API_DOCS_URL}}` | Full URL to your backend's **machine-readable** contract document — OpenAPI JSON (e.g. springdoc `/v3/api-docs`) or GraphQL introspection endpoint (or empty for FE-only). Never a swagger-ui HTML page — that's a JS shell with no endpoint data | `https://api.example.com/v3/api-docs` |
| `{{BACKEND_LOCAL_PATH}}` | Relative path from project root to a local backend checkout — contract-verification fallback when `{{API_DOCS_URL}}` is unreachable (agents read controller mappings + DTOs and record `git -C <path> log -1`). Project-relative only; empty if none | `../shop-api` |
...
| `{{WORKFLOW_PATTERNS_TABLE}}` | Markdown table of "Polished page" regression patterns the verify Workflow regression check enforces. Project fills with its specific components/hooks; default = minimal 2-row generic | (see PROFILE-GENERATOR.md crib sheet — table of project's canonical components / hooks / patterns) |
...
| `{{HARDEN_MODE_ROWS}}` | Data rows of `harden` agent's Mode table (4 rows: Component-audit / Visual-consistency / Feature-audit / Diff-harden). Project fills with its trigger phrases | (see PROFILE-GENERATOR.md crib sheet — 4 markdown table rows) |
...
| Placeholder | What it is | Example |
|---|---|---|
| `{{HARDEN_TRIGGER_KEYWORDS}}` | Comma-separated quoted triggers for `harden` agent description (multi-language allowed). Default includes `"refactor X" (DRY/consistency-flavored)` — structural refactors belong to implement | `"clean up", "DRY up X", "refactor X" (DRY/consistency-flavored), "align features X, Y, Z", "clean up the diff"` |
| `{{HARDEN_SCOPE_NOTE}}` | Optional parenthetical clarifier in `harden` description (or empty) | ` (distinct from user-global \`harden\` design skill)` |
...
| `{{IMPLEMENT_TRIGGER_KEYWORDS}}` | Comma-separated quoted triggers for `implement` agent description (multi-language allowed). Default includes `"fix X"` — implement owns bug fixes + structural refactors | `"implement X", "build feature X", "fix X", "apply this plan", "revamp X"` |
| `{{VERIFY_TRIGGER_KEYWORDS}}` | Comma-separated quoted triggers for `verify` agent description (multi-language allowed). **Triggers may be localized, but `verify` output — commit title + body, PR / push text — is always English.** | `"review my changes", "ship it", "run the checks", "draft commit"` |
...
The Report block in `implement` / `harden` / `test` agents uses small placeholders so headers translate with `{{OUTPUT_LANG}}`. `verify` always English — these are not used there.
...
|---|---|
| `{{API_DOCS_URL}}` + `{{BACKEND_LOCAL_PATH}}` (both empty) | Step 0.0 BE-scope gate (implement) + API-contract drift gate (verify). If only one is empty, the gates stay and the missing contract-source step is dropped |
| `{{LINT_STRUCTURE_CMD}}` | Shared `lint:structure` run + Structure regression check (verify) |
| `{{PAGE_STATUS_AUDIT_SOURCE}}` / `{{PAGE_STATUS_AUDIT_CMD}}` (both empty when no page-status auditor configured) | Entire Page-status check section (verify) + `{{PAGE_STATUS_AUDIT_SCRIPT_REF}}` collapses to empty |
| `{{ARCHITECTURE_DOCS_GLOB}}` / `{{COMPONENT_DOCS_GLOB}}` / `{{FEATURE_DOCS_GLOB}}` (any empty) | Matching row in verify `## Docs update` table is dropped |
